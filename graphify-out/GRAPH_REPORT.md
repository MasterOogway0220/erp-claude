...
## Corpus Check
- 472 files · ~447,521 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 786 nodes · 1053 edges · 59 communities detected
...
Cohesion: 0.07
Nodes (32): findSystemBrowser(), renderHtmlToPdf(), baseStyles(), buildChemicalTable(), buildMechanicalTable(), companyHeaderHtml(), computeChangeSnapshot(), computeCompletion() (+24 more)

### Community 3 - "Community 3"
Cohesion: 0.08
Nodes (24): convertBelowThousand(), convertToIndianWords(), convertToWesternWords(), numberToWords(), escapeHtml(), formatDate(), formatNumber(), generateInvoiceHtml() (+16 more)

### Community 4 - "Community 4"
Cohesion: 0.08
Nodes (10): generateDocumentNumber(), getCurrentFinancialYear(), getShortFinancialYear(), calculateValueDetails(), determineSupplyType(), extractStateCode(), formatDate(), generateEInvoiceJSON() (+2 more)
...
- **Thin community `Community 36`** (2 nodes): `seed-offer-terms.ts`, `main()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 37`** (2 nodes): `migrate-master-data-to-company.ts`, `main()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 38`** (2 nodes): `migrate-warehouse-details-to-heats.ts`, `main()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 39`** (2 nodes): `tmp-reset-admin.ts`, `main()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
...
- **Thin community `Community 42`** (2 nodes): `handleKeyDown()`, `handleSearch()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 43`** (2 nodes): `formatCurrency()`, `handleExportCSV()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 44`** (2 nodes): `fetchOffer()`, `handleOfferAction()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 45`** (2 nodes): `fetchReport()`, `handleUpdate()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 46`** (2 nodes): `fetchAcceptance()`, `updateStatus()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 47`** (2 nodes): `fetchAvailableStock()`, `handleOpenReservationDialog()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
...
- **Thin community `Community 49`** (2 nodes): `breadcrumbs.tsx`, `isDynamicId()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 50`** (2 nodes): `page-loading.tsx`, `PageLoading()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 51`** (2 nodes): `weight-calculation.ts`, `calculateWeightPerMeter()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
