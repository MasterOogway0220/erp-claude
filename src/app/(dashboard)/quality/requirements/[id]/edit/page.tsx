...
export default function EditQualityRequirementPage() {
  ...
  const fetchRequirement = async (id: string) => {
    try {
      const res = await fetch(`/api/quality/requirements/${id}`);
      if (!res.ok) throw new Error("Failed to fetch");
      const data = await res.json();
      setFormData({
        parameter: data.parameter || "",
        value: data.value || "",
        colourCodingRequired: data.colourCodingRequired || false,
        inspectionRequired: data.inspectionRequired || false,
        tpiAgencyId: data.tpiAgencyId || "",
        testingRequired: data.testingRequired || false,
        testType: data.testType || "",
        inspectionLocation: data.inspectionLocation || "",
        qapDocumentPath: data.qapDocumentPath || "",
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await fetch(`/api/quality/requirements/${params.id}`, {
        method: "PATCH",
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="Edit Quality Requirement"
        description={`Editing: ${formData.parameter}`}
      >
