    ...
    const newQuotation = await prisma.$transaction(async (tx) => {
      ...
      const created = await tx.quotation.create({
        data: {
          ...
          additionalDiscount: sourceQuotation.additionalDiscount,
          discountAmount: sourceQuotation.discountAmount,
          totalAfterDiscount: sourceQuotation.totalAfterDiscount,
          roundOffAmount: sourceQuotation.roundOffAmount,
          advanceToPay: sourceQuotation.advanceToPay,
          inquiryNo: sourceQuotation.inquiryNo,
          inquiryDate: sourceQuotation.inquiryDate,
          ...
          changeSnapshot: changeSnapshot ? JSON.stringify(changeSnapshot) : null,
          // Copy items
          items: {
