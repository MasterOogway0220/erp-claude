...
export function ProcessStep({ order, onComplete }: ProcessStepProps) {
  const id = order.id;
  ...
  const signalCompletion = useCallback(
    (loadedItems: ProcessingItem[]) => {
      const allProcessed =
        loadedItems.length > 0 &&
        loadedItems.every((i) => i.processing?.status === "PROCESSED");
      onComplete(allProcessed);
    },
    [onComplete]
  );
  ...
  useEffect(() => {
    // Pre-complete on mount if already PROCESSED
    if (order.processingStatus === "PROCESSED") {
      onComplete(true);
    }
  ...
  const fetchAllotmentAnalysis = async (itemId: string) => {
    try {
      const res = await fetch(
        `/api/sales-orders/${id}/allotment/analyze?itemId=${itemId}`
      );
      if (res.ok) {
        const data = await res.json();
        ...
        if (analysis) {
          ...
          setAllotmentSource(
            analysis.currentAllotment?.source || analysis.suggestedSource
          );
          if (analysis.currentAllotment) {
            setAllotmentStockQty(
              String(analysis.currentAllotment.stockQty || 0)
            );
            setAllotmentProcQty(
              String(analysis.currentAllotment.procurementQty || 0)
            );
  ...
  // -----------------------------------------------------------------------
  // Navigation (per-item sub-stepper)
  // -----------------------------------------------------------------------
  ...
  return (
    <div className="space-y-5">
      ...
      {items[currentIndex]?.processing?.status === "PROCESSED" &&
        allotmentAnalysis && (
          <Card
            className={
              allotmentConfirmed
          ...
          >
            ...
            <CardContent className="space-y-4">
              <div className="flex items-center gap-4 text-sm">
                <span>
                  Ordered:{" "}
                  <strong>{allotmentAnalysis.orderedQty} MTR</strong>
                </span>
                <span>
                  Available Stock:{" "}
                  <strong
                    className={
                      allotmentAnalysis.availableStockQty > 0
                        ? "text-green-600"
                        : "text-red-600"
                    }
                  >
              ...
              <div className="grid grid-cols-3 gap-4">
                <div className="space-y-2">
                  ...
                  >
                    ...
                    <SelectContent>
                      <SelectItem
                        value="STOCK"
                        disabled={allotmentAnalysis.availableStockQty <= 0}
                      >
                        Stock
                      </SelectItem>
                ...
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    disabled={
                      allotmentConfirmed || allotmentSource === "PROCUREMENT"
                    }
                    min={0}
                    step={0.001}
                  />
                ...
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    disabled={
                      allotmentConfirmed || allotmentSource === "STOCK"
                    }
                    min={0}
                    step={0.001}
                  />
              ...
              {!allotmentConfirmed ? (
                ...
                <div className="text-sm text-green-700 dark:text-green-400 text-center">
                  Allotment confirmed —{" "}
                  {allotmentSource === "STOCK"
                    ? "Warehouse notified"
                    : allotmentSource === "PROCUREMENT"
                      ? "Purchase Requisition created"
                      : "Warehouse notified + PR created"}
                </div>
              )}
            </CardContent>
          </Card>
