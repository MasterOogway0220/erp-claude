...
> `/api/po-acceptance/[id]/finalize` — POST
...
Operates on `pOAcceptance`, `clientPurchaseOrder`.

- **POST** — Create

## How it works

- Gated by `checkAccess("poAcceptance", "write")`. **Authentication only** — role enforcement is disabled app-wide.
