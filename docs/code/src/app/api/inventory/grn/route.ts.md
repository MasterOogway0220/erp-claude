...
Operates on `goodsReceiptNote`, `purchaseOrder`.
...
- Gated by `checkAccess("grn", "read")`, `checkAccess("grn", "write")`. **Authentication only** — role enforcement is disabled app-wide.
